...
function BiviaLines() {
  ...
  const colors = ['#C8B895', '#A89060', '#8B744A', '#6B5C35', '#777735'];
  ...
  useEffect(() => {
    ...
    const animated = svg.querySelectorAll('path[data-animated]');
    const lengths = Array.from(animated).map(p => p.getTotalLength());
    ...
    const st = ScrollTrigger.create({
      ...
      onUpdate: self => {
        animated.forEach((p, i) => {
          const delay = i * 0.08;
          const t = Math.max(0, Math.min(1, (self.progress - delay) / (1 - delay)));
    ...
    <section id="bivia-lines" ref={sectionRef} style={{ padding: '80px 0', overflow: 'hidden', position: 'sticky', top: 0, height: '100vh' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 48px', textAlign: 'center', marginBottom: '48px' }}>
        ...
        <h2 style={{ fontFamily: 'Sora,sans-serif', fontWeight: 700, fontSize: 'clamp(28px,3vw,44px)', letterSpacing: '-0.02em', lineHeight: 1.15 }}>
          Estrategia, sistemas y ejecución<br /><span style={{ color: 'var(--sand)' }}>convergiendo en un solo punto.</span>
        </h2>
      </div>
      ...
      <div style={{ position: 'relative', width: '100%', height: '440px' }}>
        <svg
          ...
          style={{ position: 'absolute', top: '-200px', left: '50%', transform: 'translateX(-50%)', width: '100%', minWidth: '900px' }}
        ...
        >
          ...
          {paths.map((d, i) => (
            <path
              ...
              strokeDashoffset="0"
              opacity="0.35"
              filter="url(#blurBivia)"
            />
          ))}

          {/* Animated sharp lines */}
          {paths.map((d, i) => (
            <path
              key={`line-${i}`}
              d={d}
